  ...
  class ApiService {
    ...
    Future<bool> login(String email, String password) async {
      ...
      try {
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          // Save user ID from JWT token
          final prefs = await SharedPreferences.getInstance();
          final userId = JwtService.getUserIdFromToken(access);
          if (userId != null) {
            await prefs.setString('user_id', userId);
            print("✅ Saved user ID: $userId");
          } else {
            print("⚠️ Could not extract user ID from token");
          }
          
          print("✅ Login successful. Access token: ${access.substring(0, 20)}...");
          return true;
        } else {
    ...
    Future<Map<String, dynamic>?> getUserMe() async {
      try {
        ...
        print("👤 User data: ${response.data}");
        print("👤 User data type: ${response.data.runtimeType}");
        print("👤 User data keys: ${response.data.keys.toList()}");
        
        // Check if imageUrl exists in the response
        if (response.data is Map<String, dynamic>) {
          final data = response.data as Map<String, dynamic>;
          print("🖼️ imageUrl in response: ${data['imageUrl']}");
          print("🖼️ All fields containing 'image': ${data.keys.where((key) => key.toLowerCase().contains('image')).toList()}");
          print("🖼️ All fields containing 'url': ${data.keys.where((key) => key.toLowerCase().contains('url')).toList()}");
        }
    ...
    Future<Response> getProfileImageByUserId(String reportId) async {
      return await _dioService.authGet('api/v1/file-upload/threads-fraud/$reportId');
    }
    ...
    Future<Map<String, dynamic>> updateUserProfile(
      ...
      try {
        ...
        try {
          ...
          print('POST request failed, trying with PUT. Error: $e');
        }
    ...
    /// Step 2: Profile Update - PUT request with firstName, lastName, and imageUrl
    ...
    Future<List<Map<String, dynamic>>> getReportsWithComplexFilter({
      ...
      try {
        print(
          '📋 Parameters: searchQuery=$searchQuery, categoryIds=$categoryIds, typeIds=$typeIds, alertLevels=$alertLevels, deviceTypeId=$deviceTypeId, detectTypeId=$detectTypeId, operatingSystemName=$operatingSystemName, startDate=$startDate, endDate=$endDate, page=$page, limit=$limit',
        );
        ...
        if (alertLevels != null && alertLevels.isNotEmpty) {
          queryParams['severityLevels'] = alertLevels;
        }
        if (deviceTypeId != null && deviceTypeId.isNotEmpty) {
          queryParams['deviceTypeId'] = deviceTypeId;
        }
        if (detectTypeId != null && detectTypeId.isNotEmpty) {
          queryParams['detectTypeId'] = detectTypeId;
        }
        if (operatingSystemName != null && operatingSystemName.isNotEmpty) {
