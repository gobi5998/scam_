...
class DueDiligenceWrapper extends StatefulWidget {
  ...
  const DueDiligenceWrapper({Key? key, this.reportId}) : super(key: key);
...
class _DueDiligenceWrapperState extends State<DueDiligenceWrapper> {
  ...
  Future<void> _uploadFile(String categoryId, String subcategoryId, FileData fileData) async {
    try {
      setState(() {
        // Show loading state
      });
      ...
      setState(() {
        uploadedFiles[categoryId]![subcategoryId]!.removeWhere((file) => file.id == fileData.id);
      });
      ...
      child: Column(
        ...
        children: [
          ...
          if (files.isNotEmpty) ...[
            ...
            const SizedBox(height: 8),
            ...files.map((fileData) => _buildFileItem(category, subcategory, fileData)).toList(),
          ],
        ],
      ),
    );
  ...
  Widget _buildFileItem(Category category, Subcategory subcategory, FileData fileData) {
    return Container(
          ...
          Row(
            children: [
              ...
              Row(
                ...
                children: [
                  IconButton(
                    onPressed: () => _uploadFile(category.id, subcategory.id, fileData),
                    icon: const Icon(Icons.cloud_upload, color: Colors.green, size: 16),
                    tooltip: 'Upload file',
                    padding: EdgeInsets.zero,
                    constraints: const BoxConstraints(),
                  ),
                  IconButton(
                    onPressed: () => _deleteFile(category.id, subcategory.id, fileData.id),
