...
class ThreadDatabaseListPage extends StatefulWidget {
  ...
  final List<Map<String, dynamic>> alertLevels;
...
class _ThreadDatabaseListPageState extends State<ThreadDatabaseListPage> {
    ...
    try {
      ...
      if (hasFilters) {
        ...
        if (widget.hasSelectedCategory &&
          ...
          queryParams['reportCategoryId'] = widget.selectedCategories.join(',');
          print('🔍 Using category ID: ${widget.selectedCategories.join(',')}');
        }

        // Add type ID if selected (use first selected type)
        if (widget.hasSelectedType && widget.selectedTypes.isNotEmpty) {
          queryParams['reportTypeId'] = widget.selectedTypes.join(',');
          print('🔍 Using type ID: ${widget.selectedTypes.join(',')}');
        }
        ...
        if (widget.hasSelectedSeverity &&
          ...
          // Pass all selected severities as comma-separated string
          queryParams['alertLevels'] = widget.selectedSeverities.join(',');
          ...
          for (String severityId in widget.selectedSeverities) {
            final selectedSeverityLevel = widget.alertLevels.firstWhere(
        ...
        // Add device, detect, and OS filters if selected
        if (widget.deviceTypeId != null && widget.deviceTypeId!.isNotEmpty) {
          queryParams['deviceTypeId'] = widget.deviceTypeId!;
          print('🔍 Using device type ID: ${widget.deviceTypeId}');
        } else {
          queryParams['deviceTypeId'] = '';
        }
        
        if (widget.detectTypeId != null && widget.detectTypeId!.isNotEmpty) {
          queryParams['detectTypeId'] = widget.detectTypeId!;
          print('🔍 Using detect type ID: ${widget.detectTypeId}');
        } else {
          queryParams['detectTypeId'] = '';
        }
        
        if (widget.operatingSystemName != null && widget.operatingSystemName!.isNotEmpty) {
          queryParams['operatingSystemName'] = widget.operatingSystemName!;
          print('🔍 Using operating system ID: ${widget.operatingSystemName}');
        } else {
        ...
        // Check if we need to use complex filter (when alert levels, device, detect, OS, or date filters are selected)
        bool needsComplexFilter =
            (widget.hasSelectedSeverity && widget.selectedSeverities.isNotEmpty) ||
            (widget.deviceTypeId != null && widget.deviceTypeId!.isNotEmpty) ||
            (widget.detectTypeId != null && widget.detectTypeId!.isNotEmpty) ||
            (widget.operatingSystemName != null && widget.operatingSystemName!.isNotEmpty) ||
        ...
        if (needsComplexFilter) {
          ...
          newReports = await _apiService.getReportsWithComplexFilter(
            ...
            detectTypeId: widget.detectTypeId,
            operatingSystemName: widget.operatingSystemName,
  ...
  Future<void> _loadFilteredReports() async {
    try {
      ...
      if (widget.isOffline || widget.localReports.isNotEmpty) {
        ...
        // Apply date filtering to local reports
        if (widget.startDate != null || widget.endDate != null) {
          reports = reports.where((report) {
            ...
            DateTime reportDate;
            if (createdAt is String) {
              reportDate = DateTime.parse(createdAt);
            ...
            if (widget.startDate != null) {
              final startOfDay = DateTime(widget.startDate!.year, widget.startDate!.month, widget.startDate!.day);
              if (reportDate.isBefore(startOfDay)) return false;
            }
          ...
          print('📱 After date filtering: ${reports.length} local reports remain');
        }
      } else {
      ...
      List<Map<String, dynamic>> base = hasActiveFilters ? _applyFilters(reports) : reports;

      // Advanced ID filters (supports String or {_id} map values)
      if (widget.deviceTypeId != null && widget.deviceTypeId!.isNotEmpty) {
        print('🔍 Applying device type filter: ${widget.deviceTypeId}');
        final beforeCount = base.length;
        base = base.where((r) {
          final v = r['deviceTypeId'] ?? r['deviceType'];
          if (v == null) return false;
          if (v is String) return v == widget.deviceTypeId;
          if (v is Map) {
            final id = v['_id']?.toString() ?? v['id']?.toString();
            return id == widget.deviceTypeId;
          }
          return false;
        }).toList();
      ...
      if (widget.detectTypeId != null && widget.detectTypeId!.isNotEmpty) {
        print('🔍 Applying detect type filter: ${widget.detectTypeId}');
        final beforeCount = base.length;
        base = base.where((r) {
          final v = r['detectTypeId'] ?? r['detectType'];
          if (v == null) return false;
          if (v is String) return v == widget.detectTypeId;
          if (v is Map) {
            final id = v['_id']?.toString() ?? v['id']?.toString();
            return id == widget.detectTypeId;
          }
          return false;
        }).toList();
      ...
      if (widget.operatingSystemName != null && widget.operatingSystemName!.isNotEmpty) {
        print('🔍 Applying operating system filter: ${widget.operatingSystemName}');
        final beforeCount = base.length;
        base = base.where((r) {
          final v = r['operatingSystemName'] ?? r['operatingSystem'];
          if (v == null) return false;
          if (v is String) return v == widget.operatingSystemName;
          if (v is Map) {
            final id = v['_id']?.toString() ?? v['id']?.toString();
            return id == widget.operatingSystemName;
          }
          return false;
        }).toList();
      ...
      // Date filter
      if (widget.startDate != null || widget.endDate != null) {
        print('📅 Applying date filter: start=${widget.startDate}, end=${widget.endDate}');
        final DateTime? sDate = widget.startDate != null
            ? DateTime(widget.startDate!.year, widget.startDate!.month, widget.startDate!.day)
            : null;
        final DateTime? eDate = widget.endDate != null
            ? DateTime(widget.endDate!.year, widget.endDate!.month, widget.endDate!.day)
            : null;

        base = base.where((r) {
          final dt = _getBestReportDate(r);
          if (dt == null) return false;
          final d = DateTime(dt.year, dt.month, dt.day);

          // Exact date match if only one date is provided or both are same day
          if (sDate != null && (eDate == null || _isSameDay(sDate, eDate))) {
            return _isSameDay(d, sDate);
          }
          if (eDate != null && sDate == null) {
            return _isSameDay(d, eDate);
          }

          // Range match (inclusive) when two different days are provided
          if (sDate != null && eDate != null) {
            final bool afterStart = !d.isBefore(sDate);
            final bool beforeEnd = !d.isAfter(eDate);
  ...
  // Resolve best date field from report
  DateTime? _getBestReportDate(Map<String, dynamic> report) {
    final candidates = [
      report['createdAt'],
      report['date'],
      report['updatedAt'],
    ...
    for (final v in candidates) {
      ...
      if (v is num) {
        try {
          ...
          final bool isSeconds = v < 1e12; // 13-digit ms vs 10-digit sec
          final dt = isSeconds
      ...
      if (v is Map) {
        try {
          // Common nested formats ('$date' must be a raw string to avoid interpolation)
          dynamic inner = v[r'$date'] ?? v['date'] ?? v['_date'] ?? v['value'];
          if (inner is String) {
            final dt = DateTime.parse(inner).toLocal();
            return dt;
          }
          if (inner is num) {
            final bool isSeconds = inner < 1e12;
            final dt = isSeconds
                ? DateTime.fromMillisecondsSinceEpoch((inner * 1000).toInt(), isUtc: true)
                : DateTime.fromMillisecondsSinceEpoch(inner.toInt(), isUtc: true);
            return dt.toLocal();
          }
          // Firestore-style {seconds, nanoseconds}
          if (v.containsKey('seconds')) {
            final seconds = (v['seconds'] as num?) ?? 0;
            final nanos = (v['nanoseconds'] as num?) ?? 0;
            final ms = (seconds * 1000).toInt() + (nanos / 1e6).round();
  ...
  List<Map<String, dynamic>> _applyFilters(List<Map<String, dynamic>> reports) {
    ...
    if (widget.hasSelectedSeverity && widget.selectedSeverities.isNotEmpty) {
      ...
      print('🔍 alertLevels available: ${widget.alertLevels.length}');

      // Pre-compute selected severity names for faster lookup
      final selectedSeverityNames = <String>[];
      for (String severityId in widget.selectedSeverities) {
        final severityLevel = widget.alertLevels.firstWhere(
              (level) => (level['_id'] ?? level['id']) == severityId,
        ...
        print('🔍 Severity ID: $severityId -> Name: $name');
      }
      print('🔍 Selected severity names: $selectedSeverityNames');

      filtered = filtered.where((report) {
        final reportSeverity = _getNormalizedAlertLevel(report);
        final reportSeverityId = _getNormalizedAlertLevelId(report);
        
        // Debug: Log what we're checking for this report
        print('🔍 Checking report: ${report['id'] ?? report['_id']}');
        print('🔍   - reportSeverity: $reportSeverity');
        print('🔍   - reportSeverityId: $reportSeverityId');
        ...
        if (reportSeverityId != null &&
          ...
          print('🔍   ✅ Matched by ID: $reportSeverityId');
          return true;
        }
        ...
        if (rawAlertLevels != null) {
          for (String selectedName in selectedSeverityNames) {
            if (rawAlertLevels == selectedName ||
              ...
              print('🔍   ✅ Matched by raw field: $rawAlertLevels == $selectedName');
              return true;
            }
          }
        }
  ...
  Future<List<Map<String, dynamic>>> _getLocalReports() async {
    ...
    for (var report in scamBox.values) {
      ...
      allReports.add({
        ...
        'emails': report
            .emails, // Use 'emails' field for consistency with backend
        'emailAddresses':
        report.emails, // Keep for backward compatibility
  ...
  void _debugFilterIssues() {
    ...
    print('🔍   - severityLevels count: ${widget.alertLevels.length}');
